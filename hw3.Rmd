...
##  [GithubLink](https://github.com/jl84785/SDS315/blob/main/hw2.Rmd)
...
The 95% confidence interval for the price difference between gas stations with and without competition ranges from -0.055 to 0.0082. Since this interval includes zero, it suggests that the price difference could be due to random variation. The price difference could range from a 5.5 cent difference to an 0.82 cent increase difference, meaning no consistent trend is evident. Because the confidence interval includes zero, there is insufficient evidence to conclude that lack of competition leads to higher gas prices. This theory is rejected.
...
The 95% confidence interval for the price difference between gas stations at stoplights and those without stoplights ranges from -0.39 to 0.31. Since this interval includes zero indicating that there is no consistent difference in gas prices between the two types of locations. The fact that zero is within the interval suggests that any observed price differences are likely due to random variation rather than a true effect of stoplight presence. The theory that gas stations at stoplights charge more is not supported by the data.
...
ggplot(pilot_survey2, aes(x=diffmean))+geom_histogram(bindwidth=.005)+labs(title = "Bootstrapping Sampling Distribution for Difference in Mean of Happy \nRatings Between The shows Living with Ed and My Name is Earl")
...
ggplot(pilot_survey4, aes(x=diffmean))+geom_histogram(bindwidth=.005)+labs(title = "Bootstrapping Sampling Distribution for differences of means of Annoyed Ratings \nbetween the Shows The Biggest Losers and The Apperentice: Los Angelos")
...
The question we are answering is if the shows Dancing with the Stars is confusing by assigning the rating 4 or 5 of being confused to just meaning that they were confused. 1-3 would indicate that they were not confused. Like all of the other question, I bootstraped the proportions of the people who thought it was confusing over the total people. We are 95% confidence that the percentage of people who were confused is around .03 to .12 percent. This suggest that most people did not find this Movie confusing.
...
We are trying to figure out if the revenue ratio between control and treatment groups have a difference. I bootstrapped the differences of the ratios between the control and treament groups. 1 indicating treatment group and 0 indicating control. We are 95% confident that the difference between the treatment group and the control group is around -.091 to -.013. This support the fact that paid search advertising on Google creates extra revenure for Ebay. 
