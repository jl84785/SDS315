    ...
    def create_adjacency_matrix(self):
        ...
        for vertex in self.vertices:
            for neighbor_idx in vertex.edges:
                adj_matrix[vertex.index][neighbor_idx] = 1
        return adj_matrix
...
def create_graph(data):
    ...
    for i in range(num_vertices):
        ...
        y = int(y.strip())
        color = color.strip()
        graph.vertices.append(ColoredVertex(i, x, y, color))
    ...
    for i in range(num_edges):
        from_idx, to_idx = map(int, lines[edge_start + 1 + i].split(","))
        graph.vertices[from_idx].add_edge(to_idx)
        graph.vertices[to_idx].add_edge(from_idx)
    # read search starting position and color
    start_info = lines[edge_start + 1 + num_edges]
    start_index, new_color = start_info.split(",")
    ...
    return graph, start_index, new_color
